...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Send Gmail notification for critical alerts
    if (newAlert.severity === 'critical') {
      const { sendClinicalAlert } = require('../utils/EmailService');
      await sendClinicalAlert(
        `Critical Medical Alert: ${newAlert.title}`,
        `<p><strong>Alert Title:</strong> ${newAlert.title}</p>
         <p><strong>Description:</strong> ${newAlert.description || 'No description provided'}</p>
         <p><strong>Severity:</strong> <span style="color:red; font-weight:bold;">CRITICAL</span></p>
